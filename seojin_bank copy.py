...
try:
    ...
    def create_account(customer_name, address, phone_number):
        # 새로운 고객 정보 추가
        cursor.execute("""
    ...
    def existing_customer(customer_name, phone_number):
        # 기존 고객 정보 조회
        cursor.execute("""
        SELECT * FROM customer
        WHERE name = %s AND phone_number = %s
        """, (customer_name, phone_number))
        customer_data = cursor.fetchone()

        # if customer_data:
        #     # 기존 고객이면 해당 고객의 계좌 생성
        #     customer_id, account_number = create_account(customer_name, phone_number, customer_data[2])
    ...
    def get_customer_info():
        customer_name = input("이름: ")
        phone_number = input("핸드폰 번호: ")
        return customer_name, phone_number


    # 신규 고객 또는 기존 고객 정보 입력 함수
    def get_customer_data():
        is_new_customer = input("신규 고객입니까? (y/n): ")

        if is_new_customer.lower() == 'y':
            # 신규 고객 정보 입력
            customer_name, address, phone_number = get_customer_info()   # 고객 정보 입력 

            # 신규 고객 계좌 생성
            customer_id = create_account(customer_name, address, phone_number)
            print(f"신규 고객이 되신 {customer_name}님의 계좌가 생성되었습니다.")
            # print(f"고객 번호: {customer_id}, 계좌 번호: {account_number}")

            return customer_id
        

        elif is_new_customer.lower() == 'n':
            # 기존 고객 정보 입력
            customer_name, phone_number = get_customer_info()

            # 기존 고객인 경우 해당 고객의 계좌 생성
            # customer_id, account_number = existing_customer(customer_name, phone_number)

            if customer_id and account_number:
                print(f"기존 고객 {customer_name}님의 계좌가 생성되었습니다.")
                # print(f"고객 번호: {customer_id}, 계좌 번호: {account_number}")

                return customer_id, 
            else:
                print("일치하는 고객 정보가 없습니다. 다시 확인해주세요.")
                return None, None
        else:
